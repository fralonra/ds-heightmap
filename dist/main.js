...
var load = __swcpack_require__.bind(void 0, function(module, exports) {
    ...
    function ds() {
        ...
        var diamond = function diamond(x, y, halfW, halfH) {
            ...
            var n = randomize(corners.reduce(function(p, c) {
                return p + c;
            }, 0) / corners.length, halfW + halfH);
            if (x < width && y < height) {
                if (n < min) min = n;
                if (n > max) max = n;
            }
        ...
        var square = function square(x, y, halfW, halfH) {
            if (data[x] === undefined || data[x][y] === undefined) return;
            var n = randomize((data[x - halfW][y - halfH] + data[x + halfW][y - halfH] + data[x + halfW][y + halfH] + data[x - halfW][y + halfH]) / 4, halfW + halfH);
            if (x < width && y < height) {
                if (n < min) min = n;
                if (n > max) max = n;
            }
        ...
        var rough = conf.rough, randomizer = conf.randomizer;
        var width = conf.width < 2 ? DEFAULT_CONFIG.width : conf.width;
        var height = conf.height < 2 ? DEFAULT_CONFIG.height : conf.height;
        ...
        function shape(sizeW, sizeH) {
            ...
            var halfW = Math.floor(sizeW / 2);
            var halfH = Math.floor(sizeH / 2);
            var x = 0;
            var y = 0;
...
function main() {
    var generate = function generate() {
        var ref = ds(config), max = ref.max, min = ref.min, data = ref.data;
        var width = data.length;
        if (width <= 0) throw "Invalid data";
        var height = data[0].length;
        var range = max - min;
        var colorData = [];
        for(var i = 0; i < height; i++)for(var j = 0; j < width; j++){
            var level = (data[j][i] - min) / range;
            if (level > 0.8) colorData.push(186, 174, 154, 255);
            else if (level > 0.6) colorData.push(222, 214, 163, 255);
            else if (level > 0.4) colorData.push(209, 215, 171, 255);
            else if (level > 0.2) colorData.push(189, 204, 150, 255);
            else colorData.push(148, 191, 139, 255);
        }
        var imageData = new ImageData(Uint8ClampedArray.from(colorData), width, height);
    ...
    if (ctx === null) throw "Canvas context is null";
    window.addEventListener("resize", function() {
        handleResize();
        generate();
    });
    var gui = new GUI$1();
    gui.add(config, "depth", 1000, 3000, 1).onChange(function(v) {
        generate();
    });
    gui.add(config, "rough", 0, 10).onChange(function(v) {
        generate();
    });
    generate();
}
